...
export default class Pitch {
	...
	public static getPitchesFromNotes(notes: Note[], octave: number, getNextOctave: boolean = false): Pitch[] {
		const lastIndex: number = getNextOctave ? 1 : 0;
		...
		for(let i = 0; i < pitchesJson.length; i++){
			...
			if(foundFirst){
				if(notes.some((n: Note) => n.name === pitchesJson[i].displayName)){
					if(pitches.length > 1 && pitchesJson[i].displayName === notes[lastIndex].name) break;
